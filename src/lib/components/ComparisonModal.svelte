            ...
            <div class="relative z-10">
                <div class="modal-header">
                    <div class="flex items-center space-x-2 md:space-x-3">
                        ...
                        <h2 class="text-lg md:text-2xl font-bold text-white drop-shadow-lg">University Comparison</h2>
                    </div>
                </div>
                <div class="modal-table-container">
                    <table class="w-full">
                        <thead class="modal-table-header">
                            <tr>
                                <th class="px-2 md:px-4 py-3 md:py-4 text-left text-xs md:text-sm font-semibold text-white/90 uppercase tracking-wider backdrop-blur-sm">Parameter</th>
                                {#each universities as university}
                                    <th class="px-2 md:px-4 py-3 md:py-4 text-left text-xs md:text-sm font-semibold text-white/90 uppercase tracking-wider backdrop-blur-sm">
                                        <div class="flex items-center space-x-3">
                                            <img src={university.logo} alt="{university.name}" class="w-8 h-8 object-contain rounded-lg bg-white/20 p-1 backdrop-blur-sm" />
                                            <span class="font-medium">{university.name}</span>
                                        </div>
                                    </th>
                                {/each}
                            </tr>
                        ...
                        <tr class="modal-table-row">
                            <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm font-medium text-white/80 backdrop-blur-sm">
                            ...
                            {#each universities as university}
                                <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm text-white/70 backdrop-blur-sm">{university.location}</td>
                            {/each}
                        </tr>
                        <tr class="modal-table-row-alt">
                            ...
                            {#each universities as university}
                                <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm text-white/70 backdrop-blur-sm">{university.fees}</td>
                            {/each}
                        </tr>
                        <tr class="modal-table-row">
                            <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm font-medium text-white/80 backdrop-blur-sm">
                            ...
                            {#each universities as university}
                                <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm text-white/70 backdrop-blur-sm">{university.ranking}</td>
                            {/each}
                        </tr>
                        <tr class="modal-table-row-alt">
                            <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm font-medium text-white/80 backdrop-blur-sm">
                            ...
                            {#each universities as university}
                                <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm text-white/70 backdrop-blur-sm">{university.accreditation}</td>
                            {/each}
                        </tr>
                        <tr class="modal-table-row">
                            <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm font-medium text-white/80 backdrop-blur-sm">
                            ...
                            {#each universities as university}
                                <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm text-white/70 backdrop-blur-sm">
                                    <div class="flex flex-wrap gap-0.5 md:gap-2">
                                        {#each university.programs as program}
                                            <span class="inline-block bg-white/20 text-white/90 px-0.5 md:px-2 py-0.5 md:py-1 rounded text-xs backdrop-blur-sm border border-white/10">{program}</span>
                                        {/each}
                                    </div>
                                </td>
                            {/each}
                        ...
                        <tr class="modal-table-row-alt">
                            ...
                            {#each universities as university}
                                <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm text-white/70 backdrop-blur-sm">{university.established}</td>
                            {/each}
                        </tr>
                        <tr class="modal-table-row">
                            <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm font-medium text-white/80 backdrop-blur-sm">
                                <div class="flex items-center space-x-1 md:space-x-2">
                                    ...
                                    <span class="hidden sm:inline">Campus Size</span>
                                    <span class="sm:hidden">Campus</span>
                                </div>
                            </td>
                            {#each universities as university}
                                <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm text-white/70 backdrop-blur-sm">{university.campusSize}</td>
                            {/each}
                        </tr>
                        <tr class="modal-table-row-alt">
                            <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm font-medium text-white/80 backdrop-blur-sm">
                                <div class="flex items-center space-x-1 md:space-x-2">
                                    <svg class="w-3 h-3 md:w-4 md:h-4 text-emerald-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z"></path>
                                    </svg>
                                    <span class="hidden sm:inline">Student Population</span>
                                    <span class="sm:hidden">Students</span>
                                </div>
                            </td>
                            {#each universities as university}
                                <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm text-white/70 backdrop-blur-sm">{university.studentPopulation}</td>
                            {/each}
                        </tr>
                        <tr class="modal-table-row">
                            <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm font-medium text-white/80 backdrop-blur-sm">
                            ...
                            {#each universities as university}
                                <td class="px-1 md:px-4 py-2 md:py-4 text-xs md:text-sm text-white/70 backdrop-blur-sm">{university.placementRate}</td>
                            {/each}
                        </tr>
                    </tbody>
                </table>
